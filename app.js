      ...
      <div class="menu-card">
        ...
        <div class="menu-card-body">
          ...
          <div class="menu-card-footer">
            ...
            <button class="btn-add-cart" onclick="addToCart(${item.id})" title="Add to cart">
...
function renderOrdersList() {
  ...
  const statusColors = { new:'#e8500a', preparing:'#d97706', delivered:'#16a34a' };
  const statusBg     = { new:'#fff4f0', preparing:'#fffbeb', delivered:'#f0fdf4' };
  const statusLabels = { new:'🆕 New Order', preparing:'🍳 Preparing', delivered:'✅ Delivered' };
  ...
  wrap.innerHTML = filtered.map(o => {
    ...
    <div class="adm-order-card" id="order-card-${o.id}" style="border-left:4px solid ${statusColors[o.status]}">
      <div class="adm-order-head">
        ...
        <span class="adm-order-status-badge" style="background:${statusBg[o.status]};color:${statusColors[o.status]};border:1px solid ${statusColors[o.status]}33">
        ...
        <div class="adm-detail-grid">
          ...
          <div class="adm-detail-row">
            ...
            <span class="adm-detail-val">
              <a href="tel:${o.phone}" style="color:var(--accent);text-decoration:none;font-weight:600;">
                <i class="bi bi-telephone-fill me-1"></i>${o.phone}
              </a>
            </span>
          </div>
        ...
        <div class="adm-items-table">
          ${o.items.map(it => `
            <div class="adm-item-row">
              ...
              <span class="adm-item-row-price">₹${it.price * it.qty}</span>
            </div>`).join('')}
          <div class="adm-item-row adm-item-total-row">
            <span class="adm-item-row-name" style="font-weight:700">Total</span>
            <span class="adm-item-row-qty"></span>
            <span class="adm-item-row-price" style="color:var(--accent);font-weight:700;font-size:1rem">₹${itemsTotal}</span>
          </div>
        </div>
      </div>
      ${o.note ? `
...
function updateOrderStatus(orderId, newStatus) {
  ...
  if (order) { order.status = newStatus; saveOrders(); renderOrdersList(); }
}
