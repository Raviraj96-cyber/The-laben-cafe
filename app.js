...
let firebaseMsg     = null;   // FCM messaging instance
let firebaseOK      = false;
let fcmToken        = null;

// FIX: Use a ready-flag so we don't alert on the initial snapshot of existing orders
...
// ── Show a real browser notification (works in foreground too) ────────────
...
function initFirebase() {
  try {
    ...
    // FCM — only works over HTTPS or localhost
    if (firebase.messaging && firebase.messaging.isSupported()) {
      firebaseMsg = firebase.messaging();

      // FIX: Foreground messages now show a real notification, not just a toast
    ...
    firebaseDB.ref('orders').on('value', snapshot => {
      ...
      fbArr.forEach(fbO => {
        ...
        if (!local) {
          orders.unshift(fbO);
          // FIX: only alert AFTER the initial snapshot has been processed
          if (fbListenerReady && !seenIds.has(fbO.id)) {
            hasNew = true;
          }
          seenIds.add(fbO.id);
        } else {
          // Update mutable fields from Firebase
          local.status  = fbO.status;
          local._fbKey  = fbO._fbKey;
        }
      });

      // Mark the listener as ready after the very first snapshot
    ...
    // Open admin panel if we were redirected from a notification tap
...
function saveOrderToFirebase(order) {
  ...
  // Mark this order as seen BEFORE it goes to Firebase so our own listener
  // doesn't ring the bell for an order we just placed ourselves.
...
async function subscribeToPushNotifications() {
  ...
  try {
    ...
    // 2) Ensure service worker is ready before getting token
    const swReg = await navigator.serviceWorker.ready;

    // 3) Get FCM token
    const token = await firebaseMsg.getToken({ vapidKey: FCM_VAPID_KEY, serviceWorkerRegistration: swReg });
    if (!token) {
    ...
    // 4) Save token to Firebase (so Cloud Function / server can push to it)
...
function updateNotifStatus() {
  ...
  if (!('Notification' in window)) {
    ...
    el.style.color = '#dc2626';
    if (btn) { btn.style.display = ''; btn.textContent = '⚙️ Open Settings'; }
  } else if (!firebaseMsg) {
    el.innerHTML   = '❌ FCM not supported (need HTTPS)';
    el.style.color = '#dc2626';
    if (btn) btn.style.display = 'none';
  } else {
...
function placeOrder(e) {
  ...
  saveOrders();
  saveOrderToFirebase(newOrder); // also marks as seen internally
...
function startEditItem(id)  { document.getElementById('adm-item-display-' + id).style.display = 'none';  document.getElementById('adm-item-edit-' + id).style.display = 'block'; }
function cancelEditItem(id) { document.getElementById('adm-item-display-' + id).style.display = 'flex';  document.getElementById('adm-item-edit-' + id).style.display = 'none'; }
...
async function saveEditItem(id) {
  ...
  if (imgInput && imgInput._base64) item.img = imgInput._base64;
...
function renderAdminList() {
  ...
  list.innerHTML = menuData.map(item => {
    ...
    // Mark the correct cat as selected
...
function confirmUpiPayment() {
  ...
  if (!uv || uv.length < 8) {
    ...
    if (ui) { ui.focus(); ui.style.borderColor = '#dc2626'; ui.style.boxShadow = '0 0 0 3px rgba(220,38,38,.15)'; setTimeout(() => { ui.style.borderColor = ''; ui.style.boxShadow = ''; }, 2500); }
    return;
  }
